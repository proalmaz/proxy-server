...
(DBAddress), m_clientFd(fd), m_DBport(DBport)
{
	...
	int flag = fcntl(m_DBFd, F_GETFL);
    fcntl(m_DBFd, F_SETFL, flag | O_NONBLOCK);
...
void Client::receiveFromClient()
{
	...
	int len = recv(m_clientFd, m_request, sizeof(m_request), MSG_NOSIGNAL);
	if (len < 0)
	{
		m_status = CLOSE;
	...
	if (m_request[4] == COM_STM_PREPARE || m_request[4] == COM_QUERY)
	{
		try
		{
...
{
	if (send(m_DBFd, m_request, m_reqLen, MSG_NOSIGNAL) < 0)
	{
...
{
	if (send(m_clientFd, m_response, m_resLen, MSG_NOSIGNAL) < 0)
	{
